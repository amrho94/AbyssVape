...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) or readfile("vape/"..scripturl) == "" then
...
local function downloadVapeAsset(path)
	if not isfile(path) or readfile(path) == "" then
		if vapeAssetTable[path] then
			vapeCachedAssets[path] = vapeAssetTable[path]
...
run(function()
	...
	function whitelist:get(plr)
		...
		for i,v in (self.data.WhitelistedUsers or {}) do
	...
	function whitelist:isplus(plr)
		return self.data.PremiumUsers and self.data.PremiumUsers[tostring(plr.UserId)] or nil
	end
	...
	function whitelist:tag(plr, text, rich)
		local plus = self:isplus(plr)
		local plrtag = ({self:get(plr)})[3] or (plus and plus.tags) or self.customtags[plr.Name] or {}
	...
	function whitelist:check(first)
		local whitelistloaded, err = pcall(function()
			local customurl = isfile('vape/whitelist-url.txt') and readfile('vape/whitelist-url.txt'):match('^%s*(.-)%s*$') or ''
			if customurl:match('^https://') and not customurl:find('YOUR%-') then
				whitelist.textdata = game:HttpGet(customurl..(customurl:find('?', 1, true) and '&' or '?')..'cache='..tostring(os.time()), true)
			else
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			...
			whitelist.localplus = whitelist:isplus(lplr) ~= nil

			for _, list in {whitelist.data.PremiumUsers or {}, whitelist.data.WhitelistedUsers or {}} do
				for i, v in list do
					if v.tags then
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				if announcement and announcement.expiretime and announcement.expiretime > os.time() then
					local targets = announcement.targets == 'all' and {tostring(lplr.UserId)} or (announcement.targets or ''):split(',')
			...
			local blacklistReason = whitelist.data.BlacklistedUsers and whitelist.data.BlacklistedUsers[tostring(lplr.UserId)]
...
run(function()
	local function destroyCape(char)
		if char then
			for _, v in pairs(char:GetDescendants()) do
				if v.Name == "Cape" or v.Name == "CapeMotor" then
					v:Destroy()
				end
			end
		end

		for _, v in pairs(gameCamera:GetChildren()) do
			if v.Name == "Cape" then
				v:Destroy()
			end
		end
	...
	local function capeFunction(char, texture)
		...
		capeSurface.Adornee = capePart
		capeSurface.Parent = capePart

		if texture and texture:lower():find("%.webm") then
			local decal = Instance.new("VideoFrame")
			decal.Name = "CapeVideo"
		...
		motor.Name = "CapeMotor"
		motor.Part0 = capePart
		motor.Part1 = torso
		motor.MaxVelocity = 0.08

		-- flipped version
		motor.C0 = CFrame.new(0, 2, 0) * CFrame.Angles(0, math.rad(90), 0)
		motor.C1 = CFrame.new(0, torso.Size.Y / 2, 0.45) * CFrame.Angles(0, math.rad(-90), 0)

		motor.Parent = capePart

		repeat
			local firstPerson = (gameCamera.CFrame.Position - gameCamera.Focus.Position).Magnitude <= 0.6
			local velocity = math.min((torso.AssemblyLinearVelocity or torso.Velocity).Magnitude, 90)

			local baseSwing = math.rad(6)
			local speedSwing = math.rad(velocity)
			local idleWave = velocity > 1 and math.abs(math.cos(tick() * 5)) / 3 or 0

			-- negative angle makes the cape bend the opposite way
			motor.DesiredAngle = -(baseSwing + speedSwing + idleWave)

			capeSurface.Enabled = not firstPerson
			capePart.Transparency = firstPerson and 1 or 0

			task.wait()
		until not capePart.Parent or not char.Parent

		destroyCape(char)
	end

	local function getCapeTexture()
		local successfulCustom

		if CapeBox and CapeBox.Value ~= "" then
			if tonumber(CapeBox.Value) then
				local suc, id = pcall(function()
					return string.match(
						game:GetObjects("rbxassetid://" .. CapeBox.Value)[1].Texture,
						"%?id=(%d+)"
					)
				end)
				...
				if not isfile(CapeBox.Value) then
					...
					successfulCustom = getcustomasset(CapeBox.Value)
				end
			end
		end

		return successfulCustom or downloadVapeAsset("vape/assets/VapeCape.png")
	end
		...
		Function = function(callback)
			if callback then
				table.insert(Cape.Connections, lplr.CharacterAdded:Connect(function(char)
					task.spawn(function()
						pcall(function()
							capeFunction(char, getCapeTexture())
						end)
					end)
				end))
				...
				if lplr.Character then
					task.spawn(function()
						pcall(function()
							capeFunction(lplr.Character, getCapeTexture())
						end)
					end)
				end
			else
