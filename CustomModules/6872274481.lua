...
-- Hash Hub crash patch: safe UI tween/toggle wrappers.
-- Some GUI/Button objects in this build can be missing methods depending on the loaded library version.
local function safeTweenSize(guiObject, targetSize, easingDirection, easingStyle, duration, override)
	if not guiObject then return end
	duration = tonumber(duration) or 0
	if duration <= 0 then
		pcall(function() guiObject.Size = targetSize end)
		return
	end
	local ok = pcall(function()
		local tween = tweenService:Create(
			guiObject,
			TweenInfo.new(duration, easingStyle or Enum.EasingStyle.Linear, easingDirection or Enum.EasingDirection.InOut),
			{Size = targetSize}
		)
		tween:Play()
	end)
...
local function safeToggleButton(buttonApi, state)
	if not buttonApi then return end
	if type(buttonApi.ToggleButton) == "function" then
		local ok = pcall(buttonApi.ToggleButton, state)
		if ok then return end
	end
	if type(buttonApi.Toggle) == "function" then
		pcall(buttonApi.Toggle, state)
	end
end
		...
		Function = function(callback)
			if callback then
				...
				if FlyAnywayProgressBarFrame and flyAllowed <= 0 and (not balloons) then 
					...
					safeTweenSize(FlyAnywayProgressBarFrame and FlyAnywayProgressBarFrame:FindFirstChild("Frame"), UDim2.new(1, 0, 0, 20), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 0, true)
				end
						...
						if flyAllowed <= 0 then 
							...
							if lastonground ~= onground then 
								if (not onground) then 
									groundtime = tick() + (2.6 + (entityLibrary.groundTick - tick()))
									if FlyAnywayProgressBarFrame then 
										safeTweenSize(FlyAnywayProgressBarFrame and FlyAnywayProgressBarFrame:FindFirstChild("Frame"), UDim2.new(0, 0, 0, 20), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, groundtime - tick(), true)
									end
								else
									if FlyAnywayProgressBarFrame then 
										safeTweenSize(FlyAnywayProgressBarFrame and FlyAnywayProgressBarFrame:FindFirstChild("Frame"), UDim2.new(1, 0, 0, 20), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 0, true)
					...
					RunLoops:BindToHeartbeat("LongJump", function(dt)
						if entityLibrary.isAlive then 
							if entityLibrary.character.Humanoid.Health <= 0 then 
								safeToggleButton(LongJump, false)
								return
							end
							...
							if changecheck ~= newval then 
								if newval then 
									safeTweenSize(LongJumpacprogressbarframe2, UDim2.new(0, 0, 0, 20), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 2.5, true)
								else
									safeTweenSize(LongJumpacprogressbarframe2, UDim2.new(1, 0, 0, 20), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 0, true)
								end
								changecheck = newval
							end
							if newval then 
