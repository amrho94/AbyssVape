...
if executorName:find("Wave") or executorName:find("macsploit") or executorName:find("Fluxus") then
...
local cacheWatermark = "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits."

local function isInvalidCachedLua(file)
	if not isfile(file) then return true end
	local suc, res = pcall(function() return readfile(file) end)
	if not suc or not res then return true end
	if res == "" then return true end
	if not file:find("%.lua$") then return false end
	res = res:gsub(cacheWatermark, ""):gsub("%s+", "")
	return res == ""
end

local function patchDownloadedScript(scripturl, source)
	if scripturl == "GuiLibrary.lua" or scripturl == "Universal.lua" then
		source = source:gsub("https://raw%.githubusercontent%.com/amrho94/AbyssVape/\"%.%.scripturl", "https://raw.githubusercontent.com/amrho94/AbyssVape/main/\"..scripturl")
	end
	if scripturl == "Universal.lua" then
		source = source:gsub("if whitelist%.data%.Announcement%.expiretime > os%.time%(%)[^\n]* then", "local announcement = whitelist.data.Announcement\n\t\t\t\tif announcement and announcement.expiretime and announcement.expiretime > os.time() then")
		source = source:gsub("whitelist%.data%.Announcement%.targets == 'all' and {tostring%(lplr%.UserId%)} or targets:split%(','%)", "announcement.targets == 'all' and {tostring(lplr.UserId)} or (announcement.targets or ''):split(',')")
		source = source:gsub("whitelist%.data%.Announcement%.text", "announcement.text or ''")
		source = source:gsub("if whitelist%.data%.BlacklistedUsers%[tostring%(lplr%.UserId%)%] then%s+task%.spawn%(lplr%.kick, lplr, whitelist%.data%.BlacklistedUsers%[tostring%(lplr%.UserId%)%]%)", "local blacklistReason = whitelist.data.BlacklistedUsers and whitelist.data.BlacklistedUsers[tostring(lplr.UserId)]\n\t\t\tif blacklistReason then\n\t\t\t\ttask.spawn(lplr.kick, lplr, blacklistReason)")
		source = source:gsub("%s*game:GetService%(\"ReplicatedStorage\"%)%[\"DefaultChatSystemChatEvents\"%]%[\"SayMessageRequest\"%]:FireServer%(\"i rape children\",\"All\"%)", "")
	elseif scripturl == "CustomModules/6872265039.lua" or scripturl:find("6872265039%.lua$") then
		if not source:find("local KnitControllers = KnitClient%.Controllers or {}", 1, false) then
			source = source:gsub("local KnitClient = debug%.getupvalue%(require%(lplr%.PlayerScripts%.TS%.knit%)%.setup, 6%)", "local KnitClient = debug.getupvalue(require(lplr.PlayerScripts.TS.knit).setup, 6)\n\t\tlocal KnitControllers = KnitClient.Controllers or {}")
		end
		source = source:gsub("KnitClient%.Controllers%.QueueController", "KnitControllers.QueueController")
		source = source:gsub("KnitClient%.Controllers%.SprintController", "KnitControllers.SprintController")
	elseif scripturl == "CustomModules/6872274481.lua" or scripturl:find("6872274481%.lua$") then
		if source:find("-- Abyss fastload patched 6872274481", 1, true) then return source end
		source = source:gsub("local KnitGotten, KnitClient\n\trepeat", "local KnitGotten, KnitClient\n\tlocal knitFetchTimeout = tick() + 3\n\trepeat")
		source = source:gsub("if KnitGotten then break end", "if KnitGotten and KnitClient then break end")
		source = source:gsub("until KnitGotten and KnitClient\n\tlocal knitStarted", "until (KnitGotten and KnitClient) or tick() > knitFetchTimeout\n\tlocal knitMissing = not (KnitGotten and KnitClient)\n\tif knitMissing then\n\t\tKnitClient = {Controllers = {}}\n\tend\n\tlocal knitStarted")
		source = source:gsub("until KnitGotten\n\trepeat task%.wait%(%) until debug%.getupvalue%(KnitClient%.Start, 1%)", "until (KnitGotten and KnitClient) or tick() > knitFetchTimeout\n\tlocal knitMissing = not (KnitGotten and KnitClient)\n\tif knitMissing then\n\t\tKnitClient = {Controllers = {}}\n\tend\n\tlocal knitStarted\n\tlocal knitStartTimeout = tick() + 2.5\n\tif not knitMissing then\n\t\trepeat\n\t\t\ttask.wait()\n\t\t\tlocal suc, res = pcall(function() return KnitClient.Start and debug.getupvalue(KnitClient.Start, 1) end)\n\t\t\tknitStarted = suc and res\n\t\tuntil knitStarted or tick() > knitStartTimeout\n\tend")
		source = source:gsub("local knitStarted\n\trepeat\n\t\ttask%.wait%(%)\n\t\tlocal suc, res = pcall%(function%(%) return KnitClient%.Start and debug%.getupvalue%(KnitClient%.Start, 1%) end%)\n\t\tknitStarted = suc and res\n\tuntil knitStarted", "local knitStarted\n\tlocal knitStartTimeout = tick() + 2.5\n\trepeat\n\t\ttask.wait()\n\t\tlocal suc, res = pcall(function() return KnitClient.Start and debug.getupvalue(KnitClient.Start, 1) end)\n\t\tknitStarted = suc and res\n\tuntil knitStarted or tick() > knitStartTimeout")
		source = source:gsub("local knitStarted\n\tlocal knitStartTimeout = tick%(%) %+ 2%.5\n\trepeat\n\t\ttask%.wait%(%)\n\t\tlocal suc, res = pcall%(function%(%) return KnitClient%.Start and debug%.getupvalue%(KnitClient%.Start, 1%) end%)\n\t\tknitStarted = suc and res\n\tuntil knitStarted or tick%(%) > knitStartTimeout", "local knitStarted\n\tlocal knitStartTimeout = tick() + 2.5\n\tif not knitMissing then\n\t\trepeat\n\t\t\ttask.wait()\n\t\t\tlocal suc, res = pcall(function() return KnitClient.Start and debug.getupvalue(KnitClient.Start, 1) end)\n\t\t\tknitStarted = suc and res\n\t\tuntil knitStarted or tick() > knitStartTimeout\n\tend")
		if not source:find("local function safeDumpRemote", 1, true) then
			source = source:gsub("local OldGet = getmetatable%(Client%)%.Get\n\tlocal OldBreak", "local OldGet = getmetatable(Client).Get\n\tlocal OldBreak\n\tlocal KnitControllers = KnitClient.Controllers or {}\n\tlocal function safeDumpRemote(callback)\n\t\tlocal suc, res = pcall(callback)\n\t\treturn suc and dumpRemote(res) or \"\"\n\tend\n\tlocal function safeRemote(callback, fallback)\n\t\tlocal remote = safeDumpRemote(callback)\n\t\treturn remote ~= \"\" and remote or fallback\n\tend\n\tlocal function safeValue(callback, fallback)\n\t\tlocal suc, res = pcall(callback)\n\t\treturn suc and res or fallback\n\tend\n\tlocal noop = function() end\n\tlocal noopItem = {Name = \"\"}\n\tlocal noopController = setmetatable({}, {__index = function(self, index)\n\t\tif index == \"heldItem\" then return noopItem end\n\t\tif index == \"Name\" then return \"\" end\n\t\treturn noop\n\tend})\n\tlocal noopPromise = {andThen = function(self, callback) return self end}\n\tlocal noopRemote = {\n\t\tinstance = {FireServer = noop, InvokeServer = noop},\n\t\tSendToServer = noop,\n\t\tCallServer = function() return nil end,\n\t\tCallServerAsync = function() return noopPromise end\n\t}\n\tlocal fallbackControllers = {\n\t\tSwordController = {\n\t\t\tlastAttack = 0,\n\t\t\tlastSwing = 0,\n\t\t\tcanSee = function() return true end,\n\t\t\tplaySwordEffect = noop,\n\t\t\tswingSwordAtMouse = noop,\n\t\t\tisClickingTooFast = function() return false end\n\t\t}\n\t}\n\tlocal function safeResolve(path, fallback)\n\t\tlocal suc, res = pcall(function() return Flamework.resolveDependency(path) end)\n\t\treturn suc and res or fallback or noopController\n\tend")
		end
		source = source:gsub("AbilityController = Flamework%.resolveDependency%(\"@easy%-games/game%-core:client/controllers/ability/ability%-controller@AbilityController\"%)", "AbilityController = safeResolve(\"@easy-games/game-core:client/controllers/ability/ability-controller@AbilityController\", {canUseAbility = function() return false end, useAbility = noop})")
		source = source:gsub("AbilityUIController = Flamework%.resolveDependency%(\"@easy%-games/game%-core:client/controllers/ability/ability%-ui%-controller@AbilityUIController\"%)", "AbilityUIController = safeResolve(\"@easy-games/game-core:client/controllers/ability/ability-ui-controller@AbilityController\")")
		source = source:gsub("CooldownController = Flamework%.resolveDependency%(\"@easy%-games/game%-core:client/controllers/cooldown/cooldown%-controller@CooldownController\"%)", "CooldownController = safeResolve(\"@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController\")")
		source = source:gsub("MatchEndScreenController = Flamework%.resolveDependency%(\"client/controllers/game/match/match%-end%-screen%-controller@MatchEndScreenController\"%)", "MatchEndScreenController = safeResolve(\"client/controllers/game/match/match-end-screen-controller@MatchEndScreenController\", {waitUntilDisplay = noop})")
		source = source:gsub("NotificationController = Flamework%.resolveDependency%('@easy%-games/game%-core:client/controllers/notification%-controller@NotificationController'%)", "NotificationController = safeResolve('@easy-games/game-core:client/controllers/notification-controller@NotificationController', {sendInfoNotification = noop})")
		source = source:gsub("AttackRemote = dumpRemote%(debug%.getconstants%(KnitClient%.Controllers%.SwordController%.sendServerRequest%)%)", "AttackRemote = safeRemote(function() return debug.getconstants(KnitControllers.SwordController.sendServerRequest) end, \"SwordHit\")")
		source = source:gsub("AttackRemote = safeDumpRemote%(function%(%) return debug%.getconstants%(KnitControllers%.SwordController%.sendServerRequest%) end%)", "AttackRemote = safeRemote(function() return debug.getconstants(KnitControllers.SwordController.sendServerRequest) end, \"SwordHit\")")
		source = source:gsub("BatteryRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(debug%.getproto%(KnitClient%.Controllers%.BatteryController%.KnitStart, 1%), 1%)%)%)", "BatteryRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(debug.getproto(KnitControllers.BatteryController.KnitStart, 1), 1)) end)")
		source = source:gsub("BlockBreaker = KnitClient%.Controllers%.BlockBreakController%.blockBreaker", "BlockBreaker = safeValue(function() return KnitControllers.BlockBreakController.blockBreaker end, {})")
		source = source:gsub("BowConstantsTable = debug%.getupvalue%(KnitClient%.Controllers%.ProjectileController%.enableBeam, 7%)", "BowConstantsTable = safeValue(function() return debug.getupvalue(KnitControllers.ProjectileController.enableBeam, 7) end, {RelX = 0, RelY = 0, RelZ = 0})")
		source = source:gsub("CannonAimRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(KnitClient%.Controllers%.CannonController%.startAiming, 5%)%)%)", "CannonAimRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(KnitControllers.CannonController.startAiming, 5)) end)")
		source = source:gsub("CannonLaunchRemote = dumpRemote%(debug%.getconstants%(KnitClient%.Controllers%.CannonHandController%.launchSelf%)%)", "CannonLaunchRemote = safeDumpRemote(function() return debug.getconstants(KnitControllers.CannonHandController.launchSelf) end)")
		source = source:gsub("ConsumeSoulRemote = dumpRemote%(debug%.getconstants%(KnitClient%.Controllers%.GrimReaperController%.consumeSoul%)%)", "ConsumeSoulRemote = safeDumpRemote(function() return debug.getconstants(KnitControllers.GrimReaperController.consumeSoul) end)")
		source = source:gsub("DamageIndicator = KnitClient%.Controllers%.DamageIndicatorController%.spawnDamageIndicator", "DamageIndicator = safeValue(function() return KnitControllers.DamageIndicatorController.spawnDamageIndicator end, noop)")
		source = source:gsub("DropItem = KnitClient%.Controllers%.ItemDropController%.dropItemInHand", "DropItem = safeValue(function() return KnitControllers.ItemDropController.dropItemInHand end, noop)")
		source = source:gsub("DropItemRemote = dumpRemote%(debug%.getconstants%(KnitClient%.Controllers%.ItemDropController%.dropItemInHand%)%)", "DropItemRemote = safeDumpRemote(function() return debug.getconstants(KnitControllers.ItemDropController.dropItemInHand) end)")
		source = source:gsub("DragonRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(debug%.getproto%(KnitClient%.Controllers%.DragonSlayerController%.KnitStart, 2%), 1%)%)%)", "DragonRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(debug.getproto(KnitControllers.DragonSlayerController.KnitStart, 2), 1)) end)")
		source = source:gsub("EatRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(KnitClient%.Controllers%.ConsumeController%.onEnable, 1%)%)%)", "EatRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(KnitControllers.ConsumeController.onEnable, 1)) end)")
		source = source:gsub("EquipItemRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(require%(replicatedStorage%.TS%.entity%.entities%[\"inventory%-entity\"%]%)%.InventoryEntity%.equipItem, 3%)%)%)", "EquipItemRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(require(replicatedStorage.TS.entity.entities[\"inventory-entity\"]).InventoryEntity.equipItem, 3)) end)")
		source = source:gsub("GuitarHealRemote = dumpRemote%(debug%.getconstants%(KnitClient%.Controllers%.GuitarController%.performHeal%)%)", "GuitarHealRemote = safeDumpRemote(function() return debug.getconstants(KnitControllers.GuitarController.performHeal) end)")
		source = source:gsub("MageRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(KnitClient%.Controllers%.MageController%.registerTomeInteraction, 1%)%)%)", "MageRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(KnitControllers.MageController.registerTomeInteraction, 1)) end)")
		source = source:gsub("PickupMetalRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(debug%.getproto%(KnitClient%.Controllers%.MetalDetectorController%.KnitStart, 1%), 2%)%)%)", "PickupMetalRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(debug.getproto(KnitControllers.MetalDetectorController.KnitStart, 1), 2)) end)")
		source = source:gsub("PickupRemote = dumpRemote%(debug%.getconstants%(KnitClient%.Controllers%.ItemDropController%.checkForPickup%)%)", "PickupRemote = safeDumpRemote(function() return debug.getconstants(KnitControllers.ItemDropController.checkForPickup) end)")
		source = source:gsub("ProjectileRemote = dumpRemote%(debug%.getconstants%(debug%.getupvalue%(KnitClient%.Controllers%.ProjectileController%.launchProjectileWithValues, 2%)%)%)", "ProjectileRemote = safeDumpRemote(function() return debug.getconstants(debug.getupvalue(KnitControllers.ProjectileController.launchProjectileWithValues, 2)) end)")
		source = source:gsub("ReportRemote = dumpRemote%(debug%.getconstants%(require%(lplr%.PlayerScripts%.TS%.controllers%.global%.report%[\"report%-controller\"%]%)%.default%.reportPlayer%)%)", "ReportRemote = safeDumpRemote(function() return debug.getconstants(require(lplr.PlayerScripts.TS.controllers.global.report[\"report-controller\"]).default.reportPlayer) end)")
		source = source:gsub("ResetRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(KnitClient%.Controllers%.ResetController%.createBindable, 1%)%)%)", "ResetRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(KnitControllers.ResetController.createBindable, 1)) end)")
		source = source:gsub("SpawnRavenRemote = dumpRemote%(debug%.getconstants%(KnitClient%.Controllers%.RavenController%.spawnRaven%)%)", "SpawnRavenRemote = safeDumpRemote(function() return debug.getconstants(KnitControllers.RavenController.spawnRaven) end)")
		source = source:gsub("TreeRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(debug%.getproto%(KnitClient%.Controllers%.BigmanController%.KnitStart, 1%), 2%)%)%)", "TreeRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(debug.getproto(KnitControllers.BigmanController.KnitStart, 1), 2)) end)")
		source = source:gsub("TrinityRemote = dumpRemote%(debug%.getconstants%(debug%.getproto%(KnitClient%.Controllers%.AngelController%.onKitEnabled, 1%)%)%)", "TrinityRemote = safeDumpRemote(function() return debug.getconstants(debug.getproto(KnitControllers.AngelController.onKitEnabled, 1)) end)")
		source = source:gsub("rawset%(self, ind, KnitClient%.Controllers%[ind%]%)", "rawset(self, ind, KnitControllers[ind] or fallbackControllers[ind] or noopController)")
		source = source:gsub("local originalRemote = OldGet%(self, remoteName%)", "if not remoteName or remoteName == \"\" then return noopRemote end\n\t\tlocal suc, originalRemote = pcall(function() return OldGet(self, remoteName) end)\n\t\tif not suc or not originalRemote then return noopRemote end")
		source = source:gsub("local tool2 = Players%.LocalPlayer%.Character:WaitForChild%(tool%.Name%)", "local localPlayer = playersService.LocalPlayer\n                        local character = localPlayer and localPlayer.Character\n                        local tool2 = character and character:FindFirstChild(tool.Name)\n                        local tool2Handle = tool2 and tool2:FindFirstChild(\"Handle\")\n                        if not tool2Handle then return end")
		source = source:gsub("model2%.CFrame = tool2:WaitForChild%(\"Handle\"%)%.CFrame %* v%.offset", "model2.CFrame = tool2Handle.CFrame * v.offset")
		source = source:gsub("weld2%.Part1 = tool2:WaitForChild%(\"Handle\"%)", "weld2.Part1 = tool2Handle")
		source = "-- Abyss fastload patched 6872274481\n"..source
	end
	return source
end
...
local function vapeGithubRequest(scripturl)
	if isInvalidCachedLua(baseDirectory..scripturl) then
		local suc, res
		...
		suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/amrho94/AbyssVape/main/"..scripturl, true) end)
		if not suc or res == "404: Not Found" then
			displayErrorPopup("Failed to connect to github : vape/"..scripturl.." : "..res)
			error(res)
		end
		res = patchDownloadedScript(scripturl, res)
		if scripturl:find(".lua") then res = cacheWatermark.."\n"..res end
		writefile(baseDirectory..scripturl, res)
	end
	local cached = readfile(baseDirectory..scripturl)
	local patched = patchDownloadedScript(scripturl, cached)
	if patched ~= cached then
		writefile(baseDirectory..scripturl, patched)
		cached = patched
	end
	return cached
end
...
local function downloadVapeAsset(path)
	if customassetcheck then
		if isInvalidCachedLua(path) then
			...
			task.spawn(function()
				...
				repeat task.wait() until not isInvalidCachedLua(path)
				textlabel:Destroy()
			end)
			local suc, req = pcall(function() return vapeGithubRequest(path:gsub("vape/assets", "assets")) end)
			if suc and req and req ~= "" then
				writefile(path, req)
			else
				return ""
			end
		...
		displayErrorPopup("Vape has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			...
			for i,v in pairs(listfiles(baseDirectory.. "CustomModules")) do
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/amrho94/AbyssVape/main/CustomModules/"..last) end)
...
local teleportConnection = playersService.LocalPlayer.OnTeleport:Connect(function(State)
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			...
			elseif isfile("vape/MainScript.lua") then
				loadfile("vape/MainScript.lua")()
			else
				loadstring(game:HttpGet("https://raw.githubusercontent.com/amrho94/AbyssVape/main/MainScript.lua", true))()
			end
		]]
...
local function loadVape()
	if not shared.VapeIndependent then
		local function safeLoadModule(source, chunkName)
			local func, compileError = loadstring(source)
			if not func then
				warn("Failed to compile "..chunkName..": "..tostring(compileError))
				return false
			end
			local suc, runtimeError = pcall(func)
			if not suc then
				warn("Failed to load "..chunkName..": "..tostring(runtimeError))
			end
			return suc
		end

		safeLoadModule(vapeGithubRequest("Universal.lua"), "Universal.lua")
		pcall(function() vapeGithubRequest("Libraries/blacklist.lua") end)
		local customModulePlaceId = game.PlaceId
		local customModuleAliases = {
			[6872265039] = 6872274481,
			[8444591321] = 6872274481
		}
		if customModuleAliases[customModulePlaceId] then
			shared.CustomSaveVape = customModuleAliases[customModulePlaceId]
			customModulePlaceId = customModuleAliases[customModulePlaceId]
		end
		if isfile(baseDirectory.. "CustomModules/"..customModulePlaceId..".lua") then
			local customModulePath = baseDirectory.. "CustomModules/"..customModulePlaceId..".lua"
			local customModuleSource = readfile(customModulePath)
			local patchedCustomModuleSource = patchDownloadedScript("CustomModules/"..customModulePlaceId..".lua", customModuleSource)
			if patchedCustomModuleSource ~= customModuleSource then
				writefile(customModulePath, patchedCustomModuleSource)
			end
			safeLoadModule(patchedCustomModuleSource, "CustomModules/"..customModulePlaceId..".lua")
		else
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/amrho94/AbyssVape/main/CustomModules/"..customModulePlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
					publicrepo = patchDownloadedScript("CustomModules/"..customModulePlaceId..".lua", publicrepo)
					writefile(baseDirectory.. "CustomModules/"..customModulePlaceId..".lua", cacheWatermark.."\n"..publicrepo)
					safeLoadModule(publicrepo, "CustomModules/"..customModulePlaceId..".lua")
				end
			end
		end
	else
	...
	if not shared.VapeSwitchServers then
		if BlatantModeToggle.Enabled then
			pcall(function()
				local frame = GuiLibrary.CreateNotification("Blatant Enabled", "Abyss is now in Blatant Mode.", 3, "assets/WarningNotification.png")
