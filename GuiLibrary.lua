...
if shared.VapeExecuted then
	...
	local function isInvalidAsset(path)
		if not isfile(path) then return true end
		local suc, res = pcall(function() return readfile(path) end)
		return (not suc) or res == nil or res == ""
	end

	local function vapeGithubRequest(scripturl)
		if isInvalidAsset("vape/"..scripturl) then
			local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/amrho94/AbyssVape/main/"..scripturl, true) end)
			assert(suc, res)
	...
	local function downloadVapeAsset(path)
		if customassetcheck then
			if isInvalidAsset(path) then
				...
				task.spawn(function()
					...
					repeat task.wait() until not isInvalidAsset(path)
					textlabel:Destroy()
				end)
		...
		if isInvalidAsset(path) then vapeCachedAssets[path] = nil end
